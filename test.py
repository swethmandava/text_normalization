...
is_train = True 
#name = 
index_to_word = json.load(open( 'vocab.txt', 'r'))
def generate_captions(pred , x) :
    global name , index_to_word
    for i,row in enumerate(pred) :
    #write input + output    
        txt = ' '.join(index_to_word[ind] for item in x[i])  + '\t' + ' '.join(index_to_word[ind] for item in row)   #+ ' ' +  word
        with open(r'results/output.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(txt)
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		BiRNN(params["num_hidden"], params["num_classes"], params["num_input"], params["learning_rate"], \
			params["encoding_layers"], params["vocab_size"], params["decoding_layers"], \
		...
		# print sess.run(tf.report_uninitialized_variables())
          saver.restore(sess, 'results/best_model.cpkt')
		for epoch in range(1) : #, params["epochs"]+1):
			...
			batch_cross_entropy_loss = batch_cross_entropy_loss / params["num_batches"]
               ...
               """
				...
				for step in range(1, params["num_valid_batches"]+1):
					...
					valid_loss = valid_loss + valid_loss_step
                        generate_captions(predicted , validX) 
		...
		plt.xlabel('Epochs')
		filename = "results/cross_entropy_model1.png"
		...
		plt.xlabel('Epochs')
		filename = "results/Perplexity_model1.png"
		plt.savefig(filename)
		plt.close()
         """ 